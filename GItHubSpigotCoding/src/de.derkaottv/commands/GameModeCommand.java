    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if( sender.isOp() ) {
            if( args.length == 1 ) {
                ...
                if( target != null ) {
                    if( args[0].equalsIgnoreCase( "0" ) || args[0].equalsIgnoreCase( "survival" ) ) {
                        if( ! target.getGameMode().equals( GameMode.SURVIVAL ) ) {
                            ...
                            sender.sendMessage( "§fThe player §6" + target.getName() + " §fis now in the §6" + target.getGameMode() + " §fmode." );
                        } else {
                ...
                sender.sendMessage( "§cPlease use: /gm <gamemode> or /gm <gamemode> <player>" );
            }
        } else {
            sender.sendMessage( "§cYou don't have permission do execute this Command!" );
        }
        return false;
    }
}
